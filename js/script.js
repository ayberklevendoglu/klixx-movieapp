const state = {
  ...
  api: {
    apiKey: "275ff089c230277879324e81a548ced4",
...
const similarMovies = async (movieId) => {
  ...
  results.forEach((movie) => {
    ...
    `;
    document.querySelector("#similar-movies").appendChild(movieCard);
  });
};
...
<div class="details-bottom">
  ...
  similarTvShows(showId);
};

const similarTvShows = async (showId) => {
  ...
  results.forEach((show) => {
    ...
    `;
    document.querySelector("#tv-similar-movies").appendChild(showCard);
  });
};
...
const search = async () => {
  ...
  state.search.type = queryString.get("type");
  state.search.term = queryString.get("search-term");
...
const displayPagination = () => {
  ...
  } else if (state.search.page === state.search.totalPages) {
...
const searchApiData = async () => {
  ...
  const response = await fetch(
    `${API_URL}search/${state.search.type}?api_key=${API_KEY}&query=${state.search.term}&page=${state.search.page}`
  );
  const data = await response.json();
  console.log(data);
  hideSpinner();
